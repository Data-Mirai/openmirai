...
### Fixed
- **An agent that used up its `max_cycles` kept being relaunched on every restart.** The scheduler unregisters it when its loop ends, but nothing cleared the durable `playing` flag, so each startup rescheduled an agent that had already finished — and `POST /stop` answered `409 "agent is not playing"`, leaving no way to clear it through the API. The scheduler now reports agents that stop on their own (`Scheduler::set_on_agent_finished`), which clears the flag; the callback fires only on self-termination, never when something outside stops the agent. `POST /stop` also clears the flag before checking whether the agent is scheduled, so stopping always disarms the relaunch even if the flag was left orphaned by a crash.
