...
pub async fn serve(
    ...
    // Mismo archivo, misma precedencia: el registro de agentes vive junto a los
    // runs. Sin esto, lo registrado por POST /agents/from-spec muere con el
    // proceso y los agentes live dejan de ciclar sin quién los relance.
    match crate::db::AgentStore::open(&resolved_db.to_string_lossy()) {
        ...
        Err(e) => {
            tracing::warn!(
                "agent registry persistence DISABLED — no se pudo abrir {}: {e}",
                resolved_db.display()
            );
        }
    }
