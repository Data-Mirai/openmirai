...
OpenMirai implements a **906-test suite** across unit, integration, and system-level tests. No external services are required—all tests run in-process with bundled SQLite and in-memory adapters.
...
| Memory & persistence | ~75 | Sync+Async | All PRs | SQLite bundled, no external |
| System & cross-cutting | ~450 | Sync+Async | All PRs | SQLite bundled, no external |
| **Total** | **906** | — | — | **Zero external dependencies** |
...
All 906 tests must pass before merge.
